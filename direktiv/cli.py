...
@click.option(
    "--search", "-s",
    help="Search documents by filename"
)
...
def stats() -> None:
    ...
    console.print(f"Documents read: {db_stats['read_documents']}/{db_stats['total_documents']}")
    
    if db_stats['total_documents'] > 0:
        read_percentage = (db_stats['read_documents'] / db_stats['total_documents']) * 100
